...
class SimpleBoosting:
    """Math: Sequential learning - M_new = M_old + alpha * residual_error"""
    def fit(self, X, y):
        self.init_pred = np.mean(y)
        residual = y - self.init_pred
        self.tree = DecisionTreeScratch()
        self.tree.fit(X, residual, task='reg')

    def predict(self, X):
        return self.init_pred + 0.1 * self.tree.predict(X)

model_10_11_12 = SimpleBoosting()
model_10_11_12.fit(X_r_train, y_r_train)